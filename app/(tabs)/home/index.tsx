...
// ✅ safest way: require() + cast fixes "string | number" errors without extra .d.ts
const HERO_BG: ImageSourcePropType = require('@/assets/padel-bg2.jpg');
...
export default function HomeScreen() {
  ...
  return (
    ...
    >
      {/* Header / Welcome (with background image) */}
      <ImageBackground
        source={HERO_BG}          // ✅ typed, not a string
        resizeMode="cover"
        imageStyle={{ borderRadius: 18 }}
        style={[styles.card, { padding: 14, borderRadius: 18, overflow: 'hidden' }]}
      >
        <View style={[styles.headerRow, { marginBottom: 10 }]}>
          <View style={{ flex: 1 }}>
            ...
            <Text style={styles.sub}>Your padel journey, gamified.</Text>
          </View>
        ...
        <View style={styles.badgeCtaRow}>
          <View style={styles.badgePill}>
            <MaterialCommunityIcons name="lightning-bolt" size={16} color="#ffb86b" />
            <Text style={styles.badgeText}>Rating</Text>
            <Text style={[styles.badgeText, { color: '#fff' }]}>
              {loadingProfile ? '—' : Math.round(profile?.rating ?? 1000)}
            </Text>
          </View>
          ...
          <View style={styles.badgePill}>
            ...
            <Text style={[styles.badgeText, { color: '#fff' }]}>{profile?.streak_days ?? 0}d</Text>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  eyebrow: { color: '#e7e7ee', fontSize: 12, marginBottom: 2, fontWeight: '700' },
  h1: { fontSize: 26, fontWeight: '800', color: '#fff' },
  sub: { color: '#f1f1f5', opacity: 0.85, marginTop: 4 },
  ...
  badgePill: {
    ...
    gap: 6,
    borderRadius: 999,
    backgroundColor: 'rgba(11,14,19,0.6)',
    borderWidth: 1,
    borderColor: '#293241',
    paddingHorizontal: 10,
    paddingVertical: 6,
  },
  badgeText: { color: '#ffb86b', fontWeight: '800', fontSize: 12 },
  badgeCtaRow: { flexDirection: 'row', gap: 8, marginTop: 10 },
  ...
  podiumRating: { fontWeight: '900', marginTop: 2, color: '#ffd79f' },
  placePill: {
    position: 'absolute',
    top: 8,
    right: 8,
